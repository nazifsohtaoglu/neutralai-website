...
## Security and PR review disposition

Additional absolute data-flow wording in How It Works, Security and Trust Center was corrected. Retention badges now say policy-dependent where the accompanying text depends on deployment configuration. PR review also requested a direct BYOK contact route while prices are hidden, synchronized stale-response testing, and fictional-data instructions in the narration. All were addressed. Narration was regenerated locally with macOS Daniel to match the revised caption text and retain audio in the walkthrough.
