...
| 2026-05-17 | Local repo checks | Growth Engineering (repo) | Pending (external portal access required) | Pending (external portal access required) | Pending (external portal access required) | Pending (external portal access required) | Pending (external portal access required) | Pending (external portal access required) | Passed (local static runtime) | `npm run build` passed; `node --test tests/content/contact-form.test.mjs tests/content/hubspot-verification-docs.test.mjs tests/content/analytics.test.mjs` passed. Browser fallback checks passed on static export served via `python3 -m http.server 3200 --directory out`: missing-config copy and script-blocked copy both rendered, and `mailto:sales@neutralai.co.uk` remained available. |
