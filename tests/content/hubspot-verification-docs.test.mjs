...
test('third-party register keeps non-secret google sheets ownership and link placeholders', () => {
  const register = readSource('docs/local/third-party-services.local.md')

  assert.match(register, /Google Sheets \+ Apps Script \| Lead capture/)
  assert.match(register, /NEXT_PUBLIC_GOOGLE_SHEETS_LEAD_ENDPOINT/)
  assert.match(register, /docs\/google-sheets-crm-setup\.md/)
  assert.match(register, /docs\/google-sheets-production-verification\.md/)
})
