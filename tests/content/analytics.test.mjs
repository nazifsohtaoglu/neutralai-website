...
test('analytics script is consent-gated and env configured', () => {
  ...
  assert.doesNotMatch(siteSource, /NEXT_PUBLIC_PLAUSIBLE_DOMAIN/)
  assert.doesNotMatch(siteSource, /NEXT_PUBLIC_PLAUSIBLE_SCRIPT_URL/)
  assert.doesNotMatch(providerSource, /<Script/)
  assert.doesNotMatch(analyticsSource, /plausible/)
...
test('posthog dashboard and funnel setup is documented', () => {
  ...
  assert.match(launchLedger, /\| Closed \| Decision recorded on 2026-05-15/)
